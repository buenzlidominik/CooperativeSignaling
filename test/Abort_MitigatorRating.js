...
contract("Abort during mitigator rating", async function(accounts) {
	...
	it("Skip State MitigatorRating", async function() {
		...
		await ProcessData.at(process).then(async function (result){
			...
			assert.equal(await result.getNextActor(), TargetAddress, "NextActor is wrong");
			assert.equal(isAtMost(await web3.eth.getBalance(TargetOwner),fundsTarget),true, "Target Funds wrong");
			assert.equal(isAtMost(await web3.eth.getBalance(MitigatorOwner),fundsMitigator),true, "Mitigator Funds wrong");
		});
    });
